...
Page({
  ...
  searhAddress: function (event) { //搜索小区并返回模糊查询结果
    ...
    wx.request({
      url: 'https://apis.map.qq.com/ws/place/v1/suggestion/?region=' + curCity + curCounty + '&region_fix=1&keyword=' + address + '&key=LCMBZ-NMFWS-XUTOR-6CNF5-TNN3Q-X3FAG',
      header: {
  ...
  tapOnSearchResult: function (event) { //点击选中小区标题修改搜索地址（ps:并且虚构一个均价与房产面积，没接口我也不想这样- -）
    ...
    wx.redirectTo({
      url: '../address-add/index?searchResult=' + searchResult + '&houseArea=' + houseArea + '&housePrice=' + housePrice + '&hasSelected=' + true + '&province=' + this.data.province + '&city=' + this.data.city + '&county=' + this.data.county + '&active=' + true
    });
